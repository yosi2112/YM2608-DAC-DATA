...
//		LOG1("  limitaddr %.6x", limitaddr);
	...
	case 0x0e:
		if ((control2 & 0x0F) == 0x0E)//sample, DA, x8 (0b00001110)
		{
			adpcmplay = false;//排他制御なので、合成を強制的に停止させる
			smpdata = 257*data;//符号付8ビット整数→符号付16ビット整数
		}
		break;
...
void OPNABase::SampMix(Sample* buffer, int nsamples)
{
	uint maskl = control2 & 0x80 ? -1 : 0;
	uint maskr = control2 & 0x40 ? -1 : 0;
...
void OPNABase::Mix6(Sample* buffer, int nsamples, int activech)
{
	...
	for (Sample* dest = buffer; dest < limit; dest+=2)
	{
		...
		//LOG2("%d %d \n", IStoSample(ibuf[2] + ibuf[3]), IStoSample(ibuf[1] + ibuf[3]));
